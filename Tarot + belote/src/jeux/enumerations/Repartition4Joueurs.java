...
import jeux.*;
public enum Repartition4Joueurs
{c1_vs_3,c2_vs_2_sans_appel,c2_vs_2_appel_au_roi,c2_vs_2_appel_a_la_figure;
	...
	{
		return name().substring(1).replace(ch_v+_+s,ch_v+vi+s).replace(ch_v+_+a,ch_v+vi+a).replace(ch_v+_+a+_,ch_v+es+ag+es).replace(_,es);
	}
}