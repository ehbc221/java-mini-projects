...
public class DSupport implements CommandListener {
    public DSupport(Display d){
     ...
     public void commandAction(Command c, Displayable d) {
       if(c == clcCommand1 ){
                  try{ TextField temp ;
                      ...
                      try{
                      in.addElement(new Double(Double.parsedouble(temp.getString())));
                      ...
                      frame2.append("minimum="+min+"\n");
                      frame2.append("maximum"+max+"\n");
                      for(int i=0;i<in.size();i++){
                          act = ((Double)in.elementAt(i)).getvalue();
                         res =(double) (act-min)/(max-min) ; 
                         frame2.append("nval-"+i+"="+res+"\n");
                      }
                 }
       ...
       if(c ==confirm){
             try{
                 ...
                 frame2.append("les valeurs des attributs\n");
                 if(tf!=null) tf = null ;
                 tf = new Vector() ;
                 for(int i=0;i<att;i++){
